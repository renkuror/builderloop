...
The browser-only portion is explicitly limited: the escalated Playwright retry reached the test runner but all 10 tests failed before assertions because `/home/user/.cache/ms-playwright/chromium_headless_shell-1187/chrome-linux/headless_shell` is missing. The initial sandboxed run also could not bind `127.0.0.1:4173` (`EPERM`). Therefore this report does not claim an interactive browser, wallet-extension, visual viewport, or console PASS. The production route, bundle, configuration, link, and on-chain checks are green.
...
| Prepared Devnet campaign | PASS | Campaign `G5NZMxbV5xgnYS2dxLBDJGAZTigMJQaH8c73tSswLLHb` is configured in the public bundle and verified on Devnet. |
...
| Return Rail | PASS (implementation/on-chain inputs) | The live branch refetches Campaign, UserProgress, and Reward from finalized Devnet RPC state before deriving the rail; no fixture scenario controls are rendered when live configuration is valid. |
...
| Runtime/console errors | LIMITED | No browser console was available in this environment. All fetched HTML/assets returned successfully; no failed production asset response was observed. |
...
- seven direct HTML routes: all HTTP 200 with the application shell;
...
| `pnpm public:verify` | PASS: 7 routes, 3 assets, 32 Explorer links |
...
- Added `scripts/verify-public-release.js` and the `pnpm public:verify` command for reproducible public route, bundle, security-marker, and Explorer-link checks.
